...
drop view if exists v_fudgemart_employees
drop view if exists v_fudgemart_approving_managers
drop view if exists v_fudgemart_pending_vacation_requests
drop view if exists v_fudgemart_approved_vacation_requests
drop table if exists fudgemart_vacation_requests
drop table if exists fudgemart_workplace_accidents
drop table if exists fudgemart_employees
drop table if exists fudgemart_departments_lookup
GO
...
go
create view v_fudgemart_employees as
	select employee_id, employee_firstname + ' ' + employee_lastname as employee_name
		from fudgemart_employees
go
create view v_fudgemart_approving_managers as
	select  e.* 
		from fudgemart_employees e 
		where e.employee_jobtitle = 'Store Manager'
		or e.employee_jobtitle = 'CEO'
go
create view v_fudgemart_pending_vacation_requests AS
	select * 
		from fudgemart_vacation_requests
	where request_approved_employee_id is null 
GO

create view v_fudgemart_approved_vacation_requests AS
	select * 
		from fudgemart_vacation_requests
	where request_approved_employee_id is not null 
GO
...
GO
insert fudgemart_vacation_requests (request_name, request_employee_id, request_start_date, request_number_of_days, request_approved_employee_id, request_approved_date)
values ( 'Watch March Madness', 7, '2020-03-17', 2, 33, '2020-03-20'),
	( 'Staycation', 15, '2020-03-23', 4, NULL, NULL),
 	( 'Go to Florida', 2, '2020-04-15', 7, 32, '2020-03-20'),
	( 'Visit Relatives', 13, '2020-04-01', 5, NULL, NULL),
	( 'Go to Hawaii', 11, '2020-04-15', 10, NULL, NULL)
